...
void saveAsOBJ(
    ...
    int loopClosureFrame1 = -1,
    int loopClosureFrame2 = -1)
{
    ...
    if (loopClosureFrame1 >= 0 && loopClosureFrame2 >= 0)
    {
        out << "# Loop closure: Frame " << loopClosureFrame1 << " <-> Frame " << loopClosureFrame2 << " (colored green)\n";
    }
    out << "\n";

    // --- Write 3D points (if enabled) ----------------------------------------
    // Points are white (1 1 1)
    if (savePoints)
    {
        out << "# 3D points (white)\n";
        for (const auto &p : points3D)
        {
            out << "v " << p.x << " " << p.y << " " << p.z << " 1 1 1\n";
        }
    }

    out << "\n# Camera centers\n";
    out << "# Regular cameras: blue (0.2 0.4 1.0)\n";
    out << "# Loop closure cameras: green (0.2 1.0 0.3)\n";

    // --- Write camera centers with colors ------------------------------------
    // Camera center C = -R^T * t
    // Regular cameras: blue, Loop closure cameras: green
    for (size_t i = 0; i < poses.size(); ++i)
    {
        ...
        // Check if this is a loop closure frame
        bool isLoopClosure = (static_cast<int>(i) == loopClosureFrame1 || 
                              static_cast<int>(i) == loopClosureFrame2);
        
        if (isLoopClosure)
        {
            // Green for loop closure frames
            out << "v " << C.at<double>(0) << " "
                        << C.at<double>(1) << " "
                        << C.at<double>(2) << " 0.2 1.0 0.3\n";
        }
        else
        {
            // Blue for regular cameras
            out << "v " << C.at<double>(0) << " "
                        << C.at<double>(1) << " "
                        << C.at<double>(2) << " 0.2 0.4 1.0\n";
        }
    }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        // Variables to track the globally best loop closure (persist for OBJ coloring)
        ...
        if (ENABLE_LOOP_CLOSURE)
        {
            ...
            // Additional variables for loop closure
        ...
        saveAsOBJ(file_name, all3DPoints, poses, SAVE_SCENE_POINTS, 
                  globalBestPastFrame, globalBestCurrFrame);
